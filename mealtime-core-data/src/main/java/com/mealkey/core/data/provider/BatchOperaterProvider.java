package com.mealkey.core.data.provider;
...
public class BatchOperaterProvider extends MapperTemplate
{
    ...
    public String updateList(MappedStatement ms)
    {
        ...
        sql.append(MealtimeSQLHelper.updateTable(entityClass, tableName(entityClass)));
        sql.append(MealtimeSQLHelper.updateSetColumns(entityClass, "record", true, isNotEmpty()));
        sql.append(MealtimeSQLHelper.wherePKColumns(entityClass, "record"));
        sql.append("</foreach>");
        return sql.toString();
    }
